...
def main():
    ...
    replis = getattr(nexus, "REPLIS_GRATUITS_PLANCHER", None)
    if not isinstance(replis, list):
        print("[RATE] REPLIS_GRATUITS absent ou pas une liste")
        sys.exit(1)
    ...
    if any(any(p in entry.lower() for p in prohibited) for entry in replis):
        print("[RATE] REPLIS_GRATUITS contient un alias facturé")
        sys.exit(1)

    # au moins un candidat LOCAL
    if not any(entry.endswith("-local") for entry in replis):
        print("[RATE] REPLIS_GRATUITS ne contient aucun modèle local")
        sys.exit(1)

    # au moins un candidat distant (cloud)
    if not any(entry.endswith("-cloud") for entry in replis):
        print("[RATE] REPLIS_GRATUITS ne contient aucun modèle distant")
        sys.exit(1)
    ...
    if not hasattr(nexus, func_name):
        print("[RATE] Fonction d'assemblage des candidats introuvable")
        sys.exit(1)
    ...
    if sample_key not in source:
        print("[RATE] Clé de bascule non détectée dans le code")
        sys.exit(1)
    ...
    if missing:
        print("[OK  ] Variables d'environnement non definies (valeurs par defaut) :", ", ".join(missing))
    else:
    ...
    if not local_models:
        print("[RATE] Aucun modèle local disponible pour le test")
        sys.exit(1)

    candidates = simulate_candidates(local_models[0])
    if any(c.endswith("-cloud") for c in candidates):
        print("[RATE] Un candidat distant apparaît avec un modèle local")
        sys.exit(1)
    ...
    for i, line in enumerate(source_lines):
        if "REPLIS_GRATUITS" in line:
            ...
            if "gratuit" in context and any(m in context for m in ("pay", "paye", "claude", "facture")):
                found_comment = True
            break

    if not found_comment:
        print("[RATE] Aucun commentaire mentionnant la gratuite et l'absence de plan paye")
        sys.exit(1)
