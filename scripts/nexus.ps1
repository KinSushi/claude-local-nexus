...
switch ($Commande.ToLower()) {
    ...
    'status' {
        & (Get-Python) (Join-Path (Join-Path (Split-Path -Parent $PSScriptRoot) "scripts") 'nexus_conformite.py') @Reste
        Exit-Avec $LASTEXITCODE
    }
    ...
    'sujets' {
        ...
        & python (Join-Path (Join-Path (Split-Path -Parent $PSScriptRoot) "scripts") 'nexus_sujets.py') @Reste
        Exit-Avec $LASTEXITCODE
    }
    'appliquer' {
        & python (Join-Path (Join-Path (Split-Path -Parent $PSScriptRoot) "scripts") 'nexus_appliquer.py') @Reste
        Exit-Avec $LASTEXITCODE
    }
    ...
    'maj-modeles' {
        ...
        # d'outil.
        & python (Join-Path (Join-Path (Split-Path -Parent $PSScriptRoot) "scripts") 'nexus_maj_modeles.py') @Reste
        Exit-Avec $LASTEXITCODE
    }
